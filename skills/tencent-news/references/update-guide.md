...
默认更新路径走平台原生脚本，不依赖 Node.js 或 Python。

在 Windows 上执行前，先检查 `scripts/_common.ps1` 和 `scripts/check-update.ps1` 是否存在；如果缺失，再读取 [`windows-powershell-templates.md`](windows-powershell-templates.md) 把它们原样写回 `scripts/` 目录，然后执行下方 PowerShell 命令。
...
sh scripts/check-update.sh --apply
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-update.ps1 -Apply
```
...
1. 确认当前平台的 CLI 已存在
2. 调用 `version`
3. 解析返回 JSON 中的 `need_update`、`current_version`、`latest_version`、`download_urls`
4. 选择当前平台对应的下载地址
5. 在应用模式下下载并覆盖旧二进制
6. 刷新 `.last-update-check-<os>-<arch>`
