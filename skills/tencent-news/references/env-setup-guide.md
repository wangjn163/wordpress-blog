...
默认使用平台原生脚本管理 API Key，不依赖 Node.js 或 Python。

在 Windows 上执行前，先检查 `scripts/_common.ps1` 和 `scripts/api-key.ps1` 是否存在；如果缺失，再读取 [`windows-powershell-templates.md`](windows-powershell-templates.md) 把它们原样写回 `scripts/` 目录，然后执行下方 PowerShell 命令。
...
```sh
sh scripts/api-key.sh
```

### Windows PowerShell

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/api-key.ps1
```
...
```sh
sh scripts/api-key.sh --set '<KEY>'
```

### Windows PowerShell

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/api-key.ps1 -Set '<KEY>'
```
...
- `storage`：写入位置
  - **macOS** (`macos-env`)：写入 shell profile + `launchctl setenv`（对 GUI 应用也生效）
  - **Linux** (`linux-shell-profile`)：写入 shell profile（zsh/bash），若 shell 不支持则回退到 `~/.profile`
...
```powershell
setx TENCENT_NEWS_APIKEY "你的Key"
$env:TENCENT_NEWS_APIKEY = '你的Key'
...
`setx` 不会更新当前 CMD 窗口，需要重新打开终端后再验证。
...
脚本已内置配置文件回退机制。`api-key.sh --set`（Windows: `api-key.ps1 -Set`）会同时写入：

1. **Shell Profile**（`~/.zshrc` / `~/.bashrc`）或 **Windows 用户环境变量** — 正常终端使用
2. **配置文件** `~/.config/tencent-news-cli/config.json` — 沙箱/跨会话回退

当 `cli-state.sh` / `cli-state.ps1` 检测到环境变量 `$TENCENT_NEWS_APIKEY` 为空时，会检查配置文件并在 JSON 输出中返回：
...
1. 正常执行 `api-key.sh --set '<KEY>'`（Windows: `api-key.ps1 -Set '<KEY>'`）— 自动写入 Shell Profile + 配置文件
2. 执行返回的 `sessionCommand` 让当前终端生效

**后续对话时（恢复）**：

1. 运行 `cli-state.sh`（Windows: `cli-state.ps1`）→ 检查 `apiKey.present`
