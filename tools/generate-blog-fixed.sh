    ...
    with open('/tmp/tavily_result.txt', 'r', encoding='utf-8') as f:
        ...
        if '## Answer' in content:
            ...
            if total_chars > 0 and chinese_chars / total_chars < 0.1:
                ...
                for sentence in sentences[:15]:  # 处理前15个句子
                ...
                # 如果提取的关键点太少，使用原始摘要
                if len(key_points) < 3:
                    print(f"⚠ 关键点提取不足({len(key_points)})，使用完整摘要", flush=True)
                    # 取前5个句子
                    summary_sentences = sentences[:5]
                    key_points = [f"• {s}" for s in summary_sentences]
                
                # 限制数量并格式化
                if key_points:
                    tavily_answer = "\n".join(key_points[:6])  # 最多6个要点
                    print(f"✓ 成功提取 {len(key_points)} 个关键点", flush=True)
                else:
                    tavily_answer = f"{answer_text[:600]}..."
