        ...
        if '## Answer' in content:
            ...
            if total_chars > 0 and chinese_chars / total_chars < 0.1:
                ...
                translations = {
                    ...
                    'advancing': '发展', 'advancements': '发展',
                    'fostering': '培育', 'highlighted': '强调',
                    'emphasizing': '强调', 'emphasized': '强调',
                    'enable': '实现', 'will enable': '将实现',
                    'shift from': '从...转向', 'shift': '转变',
                    'gaining momentum': '势头强劲', 'gaining': '获得',
                    'driven by': '由...推动', 'driven': '推动',
                    'positioning': '使...成为', 'positioned': '使...成为',
                    'unveil': '发布', 'is set to': '即将',
                    ...
                    # 形容词/副词
                    'significant': '重大', 'rapid': '快速',
                    'large-scale': '大规模', 'new': '新',
                    'substantial': '大幅', 'global': '全球',
                    'scientific': '科学', 'declining': '下降',
                ...
                for i, sentence in enumerate(sentences[:10]):
                    ...
                    if score >= 5:
                        ...
                        if len(translated) > 20:  # 确保翻译结果有意义
                            key_points.append(translated)
                            print(f"✓ 翻译句子 {i+1}: 得分={score}, 长度={len(translated)}", flush=True)

                print(f"✓ 翻译了 {len(key_points)} 个关键句", flush=True)

                # 如果翻译结果太少，使用原始摘要
                if len(key_points) < 3:
                    print("⚠ 翻译结果较少，使用原始摘要", flush=True)
                    tavily_answer = answer_text[:600] + "..."
                else:
